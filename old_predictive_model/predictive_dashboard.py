...
import pandas as pd
import time
from old_predictive_model.data_generator import AssetDataGenerator
from old_predictive_model.analytics import AssetPredictiveEngine
...
selected_asset = st.sidebar.selectbox("Select Asset to Monitor", asset_options)
...
history_key = f"history_{selected_asset.replace(' ', '_')}"
if history_key not in st.session_state:
    # Seed with 30 periods of historical normal data so charts aren't blank at launch
    st.session_state[history_key] = st.session_state.generator.generate_historical_batch(selected_asset, 30)
...
st.session_state[history_key] = history_df
